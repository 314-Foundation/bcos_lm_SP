...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.bcos:
            conv = partial(BcosConv1D, b=config.b)
            if self.is_cross_attention:
                self.c_attn = Conv1D(2 * self.embed_dim, self.embed_dim, bias=False)
                self.q_attn = Conv1D(self.embed_dim, self.embed_dim, bias=False)
            else:
                self.c_attn = Conv1D(3 * self.embed_dim, self.embed_dim, bias=False)
            self.c_proj = conv(self.embed_dim, self.embed_dim)
        else:
            if self.is_cross_attention:
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        ## bcos
        if self.bcos:
            attn_output = self.dynamic_multiplication(weight=attn_weights, input=value)
        else:
...
class GPT2FlashAttention2(GPT2Attention):
    ...
    def forward(
        ...
        logger.warning_once(
            "GPT2FlashAttention2 is not yet b-cosified. "
        )
...
class GPT2SdpaAttention(GPT2Attention):
    ...
    def forward(
        ...
        logger.warning_once(
            "GPT2SdpaAttention is not yet b-cosified. "
        )
        return super().forward(
            hidden_states,
            layer_past,
...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config):
        ...
        if hasattr(config, "bcos") and config.bcos and hasattr(config, "b"):
            conv = partial(BcosConv1D, b=config.b)
            self.act = BcosNewGELUActivation()
            self.c_fc = conv(intermediate_size, embed_dim)
            self.c_proj = conv(embed_dim, intermediate_size)
        else:
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if config.add_cross_attention:
            ...
            self.ln_cross_attn = norm(hidden_size, eps=config.layer_norm_epsilon)
...
class GPT2PreTrainedModel(PreTrainedModel, BcosModelBase):
    """
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        if hasattr(config, "bcos") and config.bcos and hasattr(config, "b"):
            ...
            norm = nn.LayerNorm
            # Note: GPT2 uses a single embedding for both tokens and positions
        self.wte = nn.Embedding(config.vocab_size, self.embed_dim)
        self.wpe = nn.Embedding(config.max_position_embeddings, self.embed_dim)

        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i) for i in range(config.num_hidden_layers)])
        self.ln_f = norm(self.embed_dim, eps=config.layer_norm_epsilon)
