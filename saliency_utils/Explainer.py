...
class BaseExplainer:
    ...
    def explain_embeddings(
        ...
        class_labels=None,
        max_length=512,
        only_predicted_classes=False,
    ):
        # if class_labels is not provided, then num_classes must be provided
        if class_labels is None:
            assert (
                num_classes is not None or only_predicted_classes
            ), "Num_classes must be provided for explainer if class_labels is not provided"
        inputs = self.tokenizer(
            texts,
            return_tensors="pt",
            padding=True,
        ...
        if "position_ids" in inputs:
            ...
            position_ids = (
                torch.arange(input_ids.size(1), dtype=torch.long, device=self.device)
                .unsqueeze(0)
        ...
        explanations = self._explain(
            ...
            only_predicted_classes=only_predicted_classes,
        )
        return explanations

    def explain_hybrid_documents_embeddings(
        self,
        text1,
        text2,
        example_indices,
        ...
        if "token_type_ids" in inputs:
            ...
            if hasattr(self.model.model, "bert"):
                token_type_ids[
                    :, input_ids.tolist().index(self.tokenizer.sep_token_id)
                ] = 1
        explanations = self._explain(
            input_ids=input_ids,
            attention_mask=attention_mask,
    ...
    def explain_hybrid_documents_tokens(
        self,
        text1,
        text2,
        example_indices,
    ...
    def explain_dataset(
        ...
        for batch in tqdm(data_loader):
            ...
            if class_labels is not None:
                batch_class_labels = [
                    predicted_label[
                        class_labels_indexer : class_labels_indexer
                        + len(example_indices)
                    ]
    ...
    def explain_hybrid_documents_dataset(
        self,
        dataset,
        num_classes=None,
        class_labels=None,
        ...
        for batch in tqdm(data_loader):
            ...
            if class_labels is not None:
                ...
                batch_class_labels = None
            explanations = self.explain_hybrid_documents(
                text1=texts1,
                text2=texts2,
                example_indices=example_indices,
                labels=labels,
...
class BcosExplainer(BaseExplainer):
    ...
    def _explain(
        ...
        if hasattr(self.model.model, "distilbert"):
            ...
            embeddings = self.model.model.roberta.embeddings(
                input_ids=input_ids, position_ids=None, token_type_ids=token_type_ids
            )
        ...
        confidences = (
            torch.nn.functional.softmax(outputs, dim=-1).detach().cpu().numpy().tolist()
        )
        ...
        for explained_labels in all_explained_labels:
            ...
            for i in range(batch_size):
                ...
                saliency_ixg_l2 = (
                    torch.norm(attributions_ixg_all[i : i + 1], dim=-1)
                    .detach()
                    .cpu()
                    .numpy()[0]
                )
                saliency_ixg_mean = (
                    attributions_ixg_all[i : i + 1]
                    .mean(dim=-1)
                    .detach()
                    .cpu()
                    .numpy()[0]
                ...
                # skip padding tokens
                tokens = [
                    token for token in tokens if token != self.tokenizer.pad_token
                ]
                ...
                result_ixg_l2 = {
                    ...
                    "text": self.tokenizer.decode(
                        [
                            ...
                            if not (
                                t in self.tokenizer.all_special_ids
                                and t != self.tokenizer.unk_token_id
                            )
                        ],
                    ...
                    "attribution": list(
                        zip(tokens, saliency_ixg_l2.tolist()[:real_length])
                    ),
                }
                ...
                result_ixg_mean = {
                    ...
                    "attribution": list(
                        zip(tokens, saliency_ixg_mean.tolist()[:real_length])
                    ),
                }
    ...
    def explain_hybrid_documents(
        ...
        return self.explain_hybrid_documents_embeddings(
            text1=text1,
            text2=text2,
            example_indices=example_indices,
            labels=labels,
...
class AttentionExplainer(BaseExplainer):
    ...
    def _explain(
        ...
        predicted_classes = (
            torch.argmax(outputs.logits, dim=1).detach().cpu().numpy().tolist()
        )
        ...
        attention_mask_matrix = (
            attention_mask_expanded * attention_mask_expanded.transpose(-1, -2)
        )
        ...
        attn_weights_sum = (
            all_attentions.sum(dim=-1, keepdim=True) + 1e-9
        ...
        # Average Attention
        # Average over heads
        avg_attn_heads = all_attentions.mean(
            dim=2
        ...
        # Attention Rollout
        rollout = (
            torch.eye(seq_len).unsqueeze(0).repeat(batch_size, 1, 1).to(self.device)
        )  # Shape: (batch_size, seq_len, seq_len)
        for attn in avg_attn_heads:
            attn = attn + torch.eye(seq_len).unsqueeze(0).to(
                self.device
            )  # Add identity for self-connections
        ...
        attn_per_layer_max = all_attentions.max(dim=2)[
            0
        ]  # Shape: (num_layers, batch_size, seq_len, seq_len)
        ...
        for i in range(batch_size):
            ...
            for class_idx in class_indices:
                ...
                raw_attention_result = {
                    ...
                    "attribution": list(
                        zip(
                            tokens[: int(valid_len)], raw_attention_attribution.tolist()
                        )
                    ),
                }
                rollout_result = {
                    ...
                    "attribution": list(
                        zip(tokens[: int(valid_len)], rollout_attribution.tolist())
                    ),
                }
                ...
                flow_result = {
                    ...
                    "attribution": list(
                        zip(tokens[: int(valid_len)], flow_attribution.tolist())
                    ),
                }
    ...
    def explain_hybrid_documents(
        ...
        return self.explain_hybrid_documents_tokens(
            text1=text1,
            text2=text2,
            example_indices=example_indices,
            labels=labels,
...
from pullbacks.attributions import PullbackAscentDiff
...
class GradientNPropabationExplainer(BaseExplainer):
    def __init__(self, model, tokenizer, method="saliency", baseline="zero"):
        ...
        if method == "Saliency":
            ...
            self.explainer = PullbackAscentDiff(self.model, pga_kwargs=pga_kwargs)
        elif method == "PullbackAscent":
            pga_kwargs = {
                "alpha": 1,
                # "steps": 10,
                "steps": 5,
    ...
    def _explain(
        ...
        for explained_labels in all_explained_labels:
            if self.method == "Saliency":
                ...
                or self.method == "DeepLift"
                or self.method == "SIG"
            ):
                if self.baseline is not None:
                    token_baseline_ids = torch.ones_like(input_ids) * self.baseline
                    if hasattr(self.model.model, "distilbert"):
                        baselines = self.model.model.distilbert.embeddings(
                            input_ids=token_baseline_ids
                        )
                    elif hasattr(self.model.model, "roberta"):
                        baselines = self.model.model.roberta.embeddings(
                            input_ids=token_baseline_ids,
                            position_ids=None,
                            token_type_ids=token_type_ids,
                        )
                    elif hasattr(self.model.model, "bert"):
                        baselines = self.model.model.bert.embeddings(
                            input_ids=token_baseline_ids,
                            position_ids=position_ids,
                            token_type_ids=token_type_ids,
                        )
            ...
            for i in range(batch_size):
                ...
                # Compute saliency metrics for each token
                saliency_l2 = (
                    torch.norm(attributions_all[i : i + 1], dim=-1)
                    .detach()
                    .cpu()
                    .numpy()[0]
                )
                saliency_mean = (
                    attributions_all[i : i + 1].mean(dim=-1).detach().cpu().numpy()[0]
                )
                # Collect results for the current example and class
                # skip padding tokens
                tokens = [
                ...
                result_l2 = {
                    ...
                    "attribution": list(
                        zip(tokens, saliency_l2.tolist()[:real_length])
                    ),
                }
        ...
        for explained_labels in all_explained_labels:
            ...
            for i in range(batch_size):
                tokens = self.tokenizer.convert_ids_to_tokens(
                    input_ids[i].detach().cpu().numpy().tolist()
                )
                class_index = explained_labels[i]
                predicted_class = predicted_classes[i]
                if labels is not None:
                    ...
                    true_label = None
                attributions_i = (
                    attributions.detach().cpu().numpy()[i]
                )  # Shape: [seq_len]
                # skip padding tokens
                tokens = [
                    token for token in tokens if token != self.tokenizer.pad_token
                ...
                result = {
                    ...
                    "method": "Occlusion",
                    "attribution": list(
                        zip(tokens, attributions_i.tolist()[:real_length])
                    ),
                }
...
class ShapleyValueExplainer(BaseExplainer):
    def __init__(
        self, model, tokenizer, method="ShapleyValue", baseline="pad", n_samples=25
    ):
        self.model = BertModelWrapper(model)
        self.model.eval()
        self.model.to(model.device)
...
class LimeExplainer(BaseExplainer):
    def __init__(
        ...
        self.prob_func = BertProbabilityModelWrapper(
            self.model, self.tokenizer, batch_size=batch_size
        )
        self.device = model.device
    ...
    def explain(
        ...
        result = {
            ...
            "method": "Lime",
            "attribution": list(
                zip(explanation.features, explanation.feature_importance.tolist())
            ),
        }
    ...
    def explain_hybrid_documents(
        ...
        # single instance
        inputs = self.tokenizer(
            text1, text2, truncation=True, max_length=max_length, return_tensors="pt"
        )
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        input_ids = inputs["input_ids"]
        attention_mask = inputs["attention_mask"]
    ...
    def explain_dataset(
        ...
        class_labels_indexer = 0
        for example in tqdm(
            batch_loader(dataset, batch_size=1, shuffle=False)
        ):  # batch operation is not supported now
            ...
            if class_labels is not None:
                batch_class_labels = [
                    predicted_label[
                        class_labels_indexer : class_labels_indexer + len(example_index)
                    ]
                    for predicted_label in class_labels
                ]
                batch_class_labels = [
                    predicted_label[0] for predicted_label in batch_class_labels
                ]
                class_labels_indexer += len(example_index)
            else:
                batch_class_labels = None
            lime_result = self.explain(
                text=text,
    ...
    def explain_hybrid_documents_dataset(
        ...
        for example in tqdm(batch_loader(dataset, batch_size=1, shuffle=False)):
            ...
            if class_labels is not None:
                ...
                batch_class_labels = None
            lime_result = self.explain_hybrid_documents(
                text1=text1,
                text2=text2,
                example_index=example_index,
                label=label,
