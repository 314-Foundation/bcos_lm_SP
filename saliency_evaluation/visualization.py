...
def print_importance(
    ...
    for importance, tokenized_text, prefix in zip(
        importances, tokenized_texts, prefixes
    ):
        if no_cls_sep:
        ...
        for i in range(len(tokenized_text)):
            ...
            if tokenized_text[i].startswith("##"):
                continue
            j = 0
            while i + j + 1 < len(tokenized_text) and tokenized_text[
                i + j + 1
            ].startswith("##"):
                j += 1
            ...
            if importance_score >= 0:
                rgba = matplotlib.colormaps.get_cmap("Greens")(
                    importance_score
                )  # Wistia
            else:
                rgba = matplotlib.colormaps.get_cmap("Reds")(
                    np.abs(importance_score)
                )  # Wistia
            text_color = (
                "color: rgba(255, 255, 255, 1.0); "
                if np.abs(importance_score) > 0.9
                else ""
            )
            color = (
                f"background-color: rgba({rgba[0]*255}, {rgba[1]*255}, {rgba[2]*255}, {rgba[3]}); "
                + text_color
            )
            html += (
                f"<span style='"
...
def print_importance_dataset(
    explanation_path, method, save_path, no_cls_sep=False, num_examples=-1
):
    ...
    for example in tqdm(explanations):
        ...
        for expl_id in range(len(example)):
            ...
            attribution_scores.append(
                [attr[1] for attr in example[expl_id]["attribution"]]
            )
            prefixes.append(f"Target Class {explained_label}:\t")
        html = print_importance(
            html=html,
            importances=attribution_scores,
